...
class Join(Command):
    ...
    def success(self):
        ...
        return "Welcome @%(username)s @%(user)s (%(user_last_name)s, %(user_first_name)s) registered to %(clinic)s clinic. Phone = %(mobile)s" % info
        #return " registered to @%(username)s (%(user_last_name)s, %(user_first_name)s) at %(clinic)s." % info
    
    def error_facility_exists(self):
        return _("The facility given does not exist.")
